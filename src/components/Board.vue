  ...
  methods: {
    ...
    formatDataForECharts(data) {
      ...
      eChartsData.forEach(item => {
        const { date, startTime, endTime, title, info1, info2, id } = item;
        const contentInfo = [
    ...
    renderItem(params, api) {
      ...
      // console.log('renderItem',dayIndex,data,dataList,startTime,endTime)
      // 获取单个数据卡片宽高
      ...
      this.$set(this.eChartsData[dataIndex], '[5]', barWidth);
    ...
    createMainContent({
      ...
      if (titleContentList.length > 1) {
        // titleFormat = '';
        let fill = '#FFFFFF';
        const elseArr = titleContentList.slice(1);
    ...
    createTitleContentList({
      ...
      const titleContent = this.createTitleContent({
        shape: titleReactShape, data: dataObj, cutLength, fill,index, isActive,
      });
      if (titleContent) {
      ...
      for (let i = 0; i < length; i += 1) {
        ...
        if (elseArr.length > 2 && i === 1) {
          titleContentList.push(this.createTitleContent({
            shape: elseTitleReactShape, data: { title: `还有${elseArr.length - 1}项...`, type: 'remind' }, cutLength, fill, index, isActive,
          }));
        } else {
          titleContentList.push(this.createTitleContent({
            shape: elseTitleReactShape, data: elseArr[i], cutLength, fill, index, isActive,
          }));
        }
      }
     ...
     */
    createTitleContent({
      shape, data, cutLength, fill, isOverlap, index, isActive,
    }) {
      if (!shape) return;
      const {
        id,
    ...
    createDataContentList({
      ...
      const titleContent = this.createTitleContent({
        shape: titleShape, data, cutLength, fill, isOverlap, index, isActive,
      });
      const dataContent = {
        type: 'group',
        children: [],
      ...
      if (barHeight >= defaultBarHeight) {
        ...
        for (let i = 0; i < 3; i += 1) {
          ...
          if (dataShape) {
            dataContent.children.push(this.createDataContent(dataShape, key, data[key], propertyCutLength, id, index));
          }
        }
      }
      return dataContent;
     ...
     */
    createDataContent(shape, key, value, cutLength, id, isActive) {
      const keyMap = {
        info1: '主持人',
        info2: '数据室',
        during: '时   间',
    ...
    tooltipMouseDown(dataInfo, event) {
      ...
      if (!dataInfo.isOwner && !this.isSystem) return;
    ...
    mouseDown(params, id) {
      ...
      console.log('dataListInOneTime',dataListInOneTime)
      ...
      const dataInfoCopy = JSON.parse(JSON.stringify(dataInfo));
      console.log('mouseDown1',dataInfoCopy)
      this.$set(this, 'dataInfoCopy', dataInfoCopy);
    ...
    mouseMove(params) {
      ...
      console.log('mouseMove4', dataList)
     ...
     */
    mouseUp() {
      // const { dataInfoCopy, data } = this;
      this.unBindDrag();
      this.$set(this, 'dataMovingShow', false);
        // const { id, date, startDay, startTime, endTime } = dataInfoCopy;
        ...
        //     endTime,
        //   };
        //   const dataInfo = data.find(item => item.id === id);
        //   if (JSON.stringify(dataInfoCopy) !== JSON.stringify(dataInfo.dataInfoCopy)) {
        //     this.updateDataTime(updateParams);
        //   }
        // }
    },
    unBindDrag() {
